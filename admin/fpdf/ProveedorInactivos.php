...
$consulta = "SELECT `nombre`, `direccion`, `telefono`, `email`, `RUC`, `Estado` FROM `proveedores` WHERE Estado='Inactivo';";
...
class PDF extends FPDF
{
   // Cabecera de página
    function Header()
    {
        ...
        $this->Cell(0, 10, 'REPORTE DE PROVEEDORES INACTIVOS', 0, 0, 'C'); // Centrar el título del reporte
        $this->Ln(20);
    }
...
foreach ($data as $dat) {
    $pdf->Cell(20, 10, '', 0, 0, 'C');
   $pdf->Cell(30, 10, utf8_decode($dat['RUC']), 1);
   $pdf->Cell(30, 10, $dat['nombre'], 1, 0, 'C');
   $pdf->Cell(60, 10, $dat['direccion'], 1, 0, 'C');
   $pdf->Cell(60, 10, $dat['email'], 1, 0, 'C');
   $pdf->Cell(30, 10, $dat['telefono'], 1, 0, 'C');
   $pdf->Cell(30, 10, $dat['Estado'], 1, 0, 'C', 1);
   $pdf->Ln();
}
...
$pdf->Output('ReporteProveedorInactivo.pdf', 'I');
?>
